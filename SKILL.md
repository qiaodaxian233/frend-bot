...
- **动工前必须 `git fetch` 核对远端 HEAD**,本地 ≠ 最新。m4 就栽了:本地闷头写了一整版 v0.3,推送才发现远端已有另一版——白干,还得靠备份分支保命。
- 撞车了怎么办:**已发布的历史赢**。弃本地、`git branch backup-xxx` 留底、reset 到远端、然后以"审查者"身份给远端版本修 bug,而不是硬推自己那版。
- 审别人(包括别的 AI 会话)的战斗代码,四个必查:①会不会打玩家/同类(红线);②状态标志有没有"只置位不复位"(本次的 retreatTicks 终身和平 bug);③公式量纲(tick 当格用,12 格隔空打人);④索敌范围是不是白名单(HostileEntity 全打会招惹末影人)。
