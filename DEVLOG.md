...
### m27 补记三:10/11 与黑匣子破案(挖矿关无罪释放)
看家关武装铁剑铁甲盾后通过(石剑裸装 1v1 尸壳是确定性的一换一,两轮 0.36799908 分毫不差——
这关考看家分支不考拳头极限,定性在案);挖矿关黑匣子一句定案:最后说=「挖了 32 块,先收工!
都在我包里」+ 扫描[匹配=1288]——测试世界地表地下遍地是石头,frend 就近挖满每单上限收工,
行为链(找矿→走位→开凿→入包→到量收工→如实汇报)完整正确,摆的两块考题石头排在一千多号
轮不上。考卷改验闭环(圆石入包=全链路通)。**黑匣子(lastSaid)+现场快照(dump)+三重筛扫描
三件取证工具留在测试基建里,以后每个新关都白得诊断力。**

### m27 收官:🎉 11/11 全绿(作者实跑 All 11 required tests passed)
七轮迭代终点:映射名(succeedWhen→Yarn 无此名)→ 单发语义(addFinalTask 实证非轮询,自建 pollUntil)
→ 平原考场(密室化+水沟)→ 批次隔离(跨考场抢目标)→ 黑匣子三件套(lastSaid/dump/三重筛)
→ 三真虫落网并修复 → 考卷两次无罪改判(红线关 bestSoFar/挖矿关 1288 块石头)。
从此:**`./gradlew runGametest` = 30 秒回归防线**,改坏砍树/挖矿/看家/种田/烧铁/寻路/红线当场知道。
自动测试之外的人味部分(钓鱼的闲情/35 天重逢那句话/LLM 闲聊)留给作者游戏里亲手验收——
它是朋友,朋友的可爱只有你能验收。

---

## 里程碑 28 / v0.27 — 多 frend 协作(压箱底大件,v0.18 挂账"分魂留待协作里程碑"兑现)

**四件套**:
1. **分魂**:灵魂档格式 v2——`{FormatV:2, LastSeenMillis, Frends:{"1":{...},"2":{...}}}`,
   每只朋友一个槽位,<b>名字/记忆/见识互不串档</b>。旧档读时视为 1 号魂原地迁移(不写盘,
   下次 save 自然升级);老实体 soulId=0 读写时视为 1,全程无感兼容。召唤分槽:在场伙伴的槽
   让开,<b>老朋友的空槽优先召回</b>(死亡/换档回来的还是"它"),都在场才开新号。
   soulId 随实体 NBT 落盘。
2. **分工(FrendCrew 认领制)**:极简共享账本——任务选中目标即认领,同伴找目标跳过已认领,
   任务结束/被打断由 FrendEntity 统一清账;60 秒过期兜底(崩服不死锁),不落盘重启自愈,
   键带维度防主世界/下界坐标重合。接线:砍树认领<b>整棵</b>(弃树时释放让给同伴——它也许有
   垫脚料)、挖矿/种田认领单块。分工不靠指挥靠认领:"你砍那棵,我砍这棵"自然涌现。
3. **点名**:`/frend name` 与聊天"你以后叫X"都只落<b>最近那只</b>(此前是全体改名);
   聊天改名分支对非最近的也吞掉该句(不该被学话/闲聊接走)。
4. **不合唱**:闲聊/LLM/兜底只有最近的接话;学话人人都学(各自记忆)但"跟你学的"得意
   只有最近的喊。<b>干活指令仍全体执行</b>——一群朋友一起应"砍树"很自然。
风味:开工时旁边有伙伴干同种活,50% 搭一句"分头干——那片归你,这片归我!"(crewChatter 可关)。
config v18→19:maxFrendsPerPlayer 默认 1→3、新增 crewChatter=true。
