...
- 当前进度:**里程碑 27 / v0.26 全自动测试已跑通**(11 关,历经五轮迭代:映射名/单发语义/平原考场/批次隔离/黑匣子取证);**自动测试首战抓获三只真虫并已修复**(手持镐不识/收工丢拾取/无主撤退站桩)。m1~m27 编译全绿。项目许可:LGPL-3.0。日常回归:`./gradlew runGametest` 一条命令。
